...
// RUN: split-file %s %t
// RUN: %jforce-opt %t/assign.mlir --jforce-annotate | FileCheck %s --check-prefix=ASSIGN
// RUN: %jforce-opt %t/clover.mlir --jforce-annotate | FileCheck %s --check-prefix=CLOVER

//--- assign.mlir
// ASSIGN-LABEL: func.func @kernel
// ASSIGN-SAME: %arg4: !fir.ref<i32> {jit.literal_val = 128 : i64, jit.shape_arg}
// ASSIGN-SAME: %arg5: !fir.ref<i32> {jit.literal_val = 128 : i64, jit.shape_arg}
// ASSIGN-SAME: %arg6: !fir.ref<i32> {jit.literal_val = 1 : i64, jit.shape_arg}
// ASSIGN-SAME: %arg7: !fir.ref<i32> {jit.literal_val = 128 : i64, jit.shape_arg}
// ASSIGN-SAME: %arg8: !fir.ref<i32> {jit.literal_val = 1 : i64, jit.shape_arg}
module {
  func.func @kernel(%arg0: !fir.ref<i32> {jit.literal_val = 0 : i64}, %arg1: !fir.ref<i32> {jit.literal_val = 128 : i64}, %arg2: !fir.ref<!fir.array<?xf64>>, %arg3: !fir.ref<!fir.array<?xf64>>, %arg4: !fir.ref<i32> {jit.literal_val = 128 : i64}, %arg5: !fir.ref<i32> {jit.literal_val = 128 : i64}, %arg6: !fir.ref<i32> {jit.literal_val = 1 : i64}, %arg7: !fir.ref<i32> {jit.literal_val = 128 : i64}, %arg8: !fir.ref<i32> {jit.literal_val = 1 : i64}) {
    %c0 = arith.constant 0 : index
    %0 = fir.alloca i32
    %1 = fir.load %arg0 : !fir.ref<i32>
    fir.store %1 to %0 : !fir.ref<i32>
    %2 = fir.load %arg8 : !fir.ref<i32>
    %3 = fir.load %arg7 : !fir.ref<i32>
    %4 = fir.load %arg6 : !fir.ref<i32>
    %5 = fir.load %arg5 : !fir.ref<i32>
    %6 = fir.load %arg4 : !fir.ref<i32>
    %7 = fir.convert %5 : (i32) -> index
    %8 = arith.cmpi sgt, %7, %c0 : index
    %9 = fir.convert %6 : (i32) -> index
    %10 = arith.cmpi sgt, %9, %c0 : index
    %11 = arith.select %10, %9, %c0 : index
    %12 = arith.select %8, %7, %c0 : index
    %13:2 = hlfir.declare %0 {uniq_name = "_QFFrun_benchmarkEi"} : (!fir.ref<i32>) -> (!fir.ref<i32>, !fir.ref<i32>)
    %14:2 = hlfir.declare %arg1 {uniq_name = "_QFFrun_benchmarkEn"} : (!fir.ref<i32>) -> (!fir.ref<i32>, !fir.ref<i32>)
    %15 = fir.shape %12 : (index) -> !fir.shape<1>
    %16:2 = hlfir.declare %arg2(%15) {uniq_name = "_QFFrun_benchmarkEy"} : (!fir.ref<!fir.array<?xf64>>, !fir.shape<1>) -> (!fir.box<!fir.array<?xf64>>, !fir.ref<!fir.array<?xf64>>)
    %17 = fir.shape %11 : (index) -> !fir.shape<1>
    %18:2 = hlfir.declare %arg3(%17) {uniq_name = "_QFFrun_benchmarkEx"} : (!fir.ref<!fir.array<?xf64>>, !fir.shape<1>) -> (!fir.box<!fir.array<?xf64>>, !fir.ref<!fir.array<?xf64>>)
    %19 = fir.load %13#0 : !fir.ref<i32>
    %20 = fir.alloca i32
    fir.store %19 to %20 : !fir.ref<i32>
    %21:2 = hlfir.declare %20 {uniq_name = "_QFFrun_benchmarkEi"} : (!fir.ref<i32>) -> (!fir.ref<i32>, !fir.ref<i32>)
    %22 = fir.convert %4 : (i32) -> index
    %23 = fir.convert %3 : (i32) -> index
    %24 = fir.convert %2 : (i32) -> index
    fir.do_loop %arg9 = %22 to %23 step %24 {
      hlfir.assign %arg9 to %21#0 : index, !fir.ref<i32>
      %25 = fir.load %21#0 : !fir.ref<i32>
      %26 = fir.convert %25 : (i32) -> i64
      %27 = hlfir.designate %18#0 (%26)  : (!fir.box<!fir.array<?xf64>>, i64) -> !fir.ref<f64>
      %28 = fir.load %27 : !fir.ref<f64>
      %29 = fir.load %21#0 : !fir.ref<i32>
      %30 = fir.convert %29 : (i32) -> i64
      %31 = hlfir.designate %16#0 (%30)  : (!fir.box<!fir.array<?xf64>>, i64) -> !fir.ref<f64>
      hlfir.assign %28 to %31 : f64, !fir.ref<f64>
    }
    omp.terminator
  }
}
