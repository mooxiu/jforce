program test_dot_product
  ...
  integer(c_long), parameter :: n = 100
  real(c_float), allocatable, target :: a(:), b(:), out(:)
  integer :: i

  allocate(a(n), b(n), out(1))
  ...
  block
    ...
    integer(c_long), target:: shape_Out(1) = 1
  ...
  ! Verify with ghci by: "sum (map (\x -> x * x) [1..n])" 
