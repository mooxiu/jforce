...
struct FirLICMPass
    : public mlir::PassWrapper<FirLICMPass,
                               mlir::OperationPass<mlir::func::FuncOp>> {

  // The return value could be a nullptr.
  static Operation *getOutermostLoopNotContainingDefOp(func::FuncOp funcOp,
                                                       fir::LoadOp loadOp,
                                                       Operation *defOp) {
    Operation *currLoop = nullptr;
    Operation *currOp = loadOp;
    while (currOp != funcOp && !currOp->isAncestor(defOp)) {
      if (llvm::isa<affine::AffineForOp, scf::ForOp, fir::DoLoopOp>(currOp)) {
        currLoop = currOp;
      }
  ...
  static void moveFirLoad(func::FuncOp funcOp, OpBuilder &opBuilder) {
    ...
    funcOp.walk([&](fir::LoadOp loadOp) {
      ...
      if (llvm::isa<hlfir::DeclareOp>(defOp)) {
        auto outermostLoop =
            getOutermostLoopNotContainingDefOp(funcOp, loadOp, defOp);
        if (outermostLoop != nullptr) {
          opBuilder.setInsertionPoint(outermostLoop);
          // static LoadOp create(::mlir::OpBuilder &builder, ::mlir::Location
          // location, mlir::Value refVal);
          auto movedLoadOp = fir::LoadOp::create(
              opBuilder, outermostLoop->getLoc(), loadOp.getMemref());
  ...
  StringRef getArgument() const override { return "jforce-fir-licm"; }
...
void registerFirLICMPass() {
  ::mlir::registerPass(
      []() -> std::unique_ptr<mlir::Pass> { return createFirLICMPass(); });
};
} // namespace xla_jit
