...
// The kernel function outlined a non-stablehlo function named
// `outlined_affinefor_[0-9]+`, which is the `outlinedFuncName`. It is supposed
// to be transformed by EnzymeJAX's passes to become
// `outlined_affinefor_[0-9]+_raised`.
//
// @retval: empty if current function we are checking is not the outlined
// function.
// @retval: Some(name) if current function is outlined function, we return the
// corresponding StableHLO function's name.
static std::string
getCorrespondingStableHLOFuncName(::mlir::StringRef outlinedFuncName) {
  std::regex re(
...
struct ReplaceOutlineFuncCall : public OpRewritePattern<func::FuncOp> {
  ...
  static void replaceMemrefFuncCall(ModuleOp moduleOp,
                                    PatternRewriter &rewritter,
                                    func::CallOp callOp,
                                    ::mlir::StringRef outlinedFuncName) {
    func::FuncOp outlinedFunc =
        moduleOp.lookupSymbol<func::FuncOp>(outlinedFuncName);
    ...
    for (int paramIdx = 0; paramIdx < paramsSize; paramIdx++) {
      ...
      MemRefType operandMemrefType =
          llvm::dyn_cast<MemRefType>(memrefFuncParam.getType());
      auto operandTensorType = RankedTensorType::get(
          operandMemrefType.getShape(), operandMemrefType.getElementType());
      auto memrefTypeToTensorTypeOp = bufferization::ToTensorOp::create(
          rewritter, callOp.getLoc(), operandTensorType, memrefFuncParam);
      tensorParams.push_back(memrefTypeToTensorTypeOp);
    }
    auto hloFuncCallOp =
        func::CallOp::create(rewritter, callOp.getLoc(), hloFunc, tensorParams);

    rewritter.setInsertionPointAfter(callOp);
    for (int paramIdx = 0; paramIdx < paramsSize; paramIdx++) {
      auto hloFuncCallOpRes = hloFuncCallOp.getResult(paramIdx);
      // memref.tensor_store has been replaced with
      // bufferization.materialize_in_destination, see:
      // https://github.com/llvm/llvm-project/pull/71010
      if (llvm::isa<MemRefType>(memrefFuncParams[paramIdx].getType())) {
        bufferization::MaterializeInDestinationOp::create(
            rewritter, callOp.getLoc(), mlir::TypeRange{}, hloFuncCallOpRes,
            memrefFuncParams[paramIdx], false, true);
      }
    }
  ...
  LogicalResult matchAndRewrite(func::FuncOp funcOp,
    ...
    funcOp.walk([&](func::CallOp callOp) {
      if (isNotOutlineFunc(callOp.getCallee())) {
        // This call might be call some other built-in functions rather than
        // built-in functions.
...
struct SwitchOutlineFuncPass
    : public mlir::PassWrapper<SwitchOutlineFuncPass, OperationPass<ModuleOp>> {
...
void registerRemergePass() {
  ::mlir::registerPass(
      []() -> std::unique_ptr<mlir::Pass> { return createRemergePass(); });
};
} // namespace xla_jit
