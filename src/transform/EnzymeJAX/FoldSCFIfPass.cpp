...
static bool hasSingleStore(Block *block) {
  llvm::SetVector<mlir::Value> memrefs;

  for (Operation &op : block->getOperations()) {
    if (isa<mlir::affine::AffineStoreOp, memref::StoreOp>(op)) {
      mlir::Value memref = op.getOperand(1);
      ...
      if (auto storeOp = dyn_cast<mlir::affine::AffineStoreOp>(op)) {
        if (llvm::any_of(storeOp.getMapOperands(), [&](mlir::Value operand) {
              return operand.getParentBlock() == block;
            }))
          return false;
      } else if (auto storeOp = dyn_cast<memref::StoreOp>(op)) {
        if (llvm::any_of(storeOp.getIndices(), [&](mlir::Value operand) {
              return operand.getParentBlock() == block;
            }))
          return false;
      }
...
static bool hasMatchingStores(ArrayRef<Block *> blocks) {
  ...
  llvm::SetVector<mlir::Value> setUnion;
...
static void getMemRefStoreInfo(Block *block,
                               llvm::MapVector<mlir::Value, MemRefStoreInfo> &storeInfo) {
  unsigned ord = 0;
  for (Operation &op : block->getOperations())
    if (isa<memref::StoreOp, mlir::affine::AffineStoreOp>(op)) {
...
static LogicalResult liftStoreOps(scf::IfOp ifOp, func::FuncOp f,
  ...
  auto cloneBlock = [&](Block *target, Block *source) {
    ...
    unsigned numExistingResults = yieldOp.getResults().size();
    SmallVector<mlir::Value> results(numExistingResults + storeInfo.size());
    ...
    for (Operation &op : source->getOperations()) {
      if (isa<memref::StoreOp, mlir::affine::AffineStoreOp>(op)) {
        mlir::Value memref = op.getOperand(1);
        mlir::Value toStore = op.getOperand(0);
...
static bool foldSCFIf(scf::IfOp ifOp, func::FuncOp f, OpBuilder &b) {
  ...
  LLVM_DEBUG(llvm::dbgs() << "Working on ifOp: " << ifOp << "\n\n");
...
struct FoldSCFIfPass: mlir::PassWrapper<FoldSCFIfPass, OperationPass<func::FuncOp>> {
  void getDependentDialects(DialectRegistry &registry) const override {
    return;
  }

  StringRef getArgument() const override { return "enzyme-fold-scf-if"; }

  void runOnOperation() override {
    auto funcOp = getOperation();
    auto ctx = getOperation()->getContext();
    IRRewriter rewriter(funcOp.getContext());
    processLiftStoreOps(funcOp, rewriter);
    process(funcOp, rewriter);
    return;
  }
};
...
std::unique_ptr<mlir::Pass> createFoldSCFIfPass() {
  return std::make_unique<FoldSCFIfPass>();
}

void registerFoldSCFIfPass() {
  ::mlir::registerPass([]() -> std::unique_ptr<mlir::Pass> {
    return createFoldSCFIfPass();
  });
};
} // namespace xla_jit
