// INFO: This is copied and adapted from Enzyme-JAX
...
void PolygeistMem2Reg::runOnOperation() {
  // auto *f = getOperation();
  // INFO: explicitly declare a function pass here when adapting, so I removed the pointer. 
  auto f = getOperation();
...
void registerPolygeistMem2RegPass() {
  ::mlir::registerPass(
      []() -> std::unique_ptr<mlir::Pass> { return createPolygeistMem2RegPass(); });
};
} // namespace xla_jit
