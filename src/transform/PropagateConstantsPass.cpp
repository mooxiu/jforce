...
namespace {
  ...
  static bool foldLoadOpWithConstant(OpBuilder& opBuilder, fir::LoadOp loadOp, int constantVal) {
    ...
    arith::ConstantIntOp cop = arith::ConstantIntOp::create(opBuilder, loadOp.getLoc(), resValue.getType(), constantVal);
  ...
  struct PropagateConstantsPass:
    public mlir::PassWrapper<PropagateConstantsPass, mlir::OperationPass<func::FuncOp>> {
    ...
    void runOnOperation() override {
      ...
      funcOp.walk([&](hlfir::DeclareOp declareOp){
        ...
        if (declaredVal.second) {
          // This declareOp declares a constant value, and it will be used as shape or bound.
          // I should find the usages of it and change to the value.
...
namespace xla_jit {
  ...
  void registerPropagateConstantsPass() {
    ::mlir::registerPass([]()->std::unique_ptr<mlir::Pass>{return createPropagateConstantsPass();});
  };
}
