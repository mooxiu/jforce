...
// Posted by Czarek Tomczak, modified by community. See post 'Timeline' for
// change history Retrieved 2025-11-24, License - CC BY-SA 3.0
...
std::string GetTransposeOp(std::vector<int64_t> shape) {
  ...
  replace_all(op, "{SHAPE1}",
              std::to_string(shape[0]) + "x" + std::to_string(shape[1]));
  replace_all(op, "{SHAPE2}",
              std::to_string(shape[1]) + "x" + std::to_string(shape[0]));
