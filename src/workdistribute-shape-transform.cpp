...
/// Some arguments are there just meant to be shape meta data, need to drop them
/// for better performance. Return a map mapping original Index -> new Index;
...
trimShapeArgs(MLIRContext *context, func::FuncOp &funcOp, int64_t *ArgTypes) {
  ...
  for (int oldIdx = 0; oldIdx < funcOp.getNumArguments(); oldIdx++) {
    if (!isLiteralTy(ArgTypes[oldIdx]) ||
        argsToKeep.contains(funcOp.getArgument(oldIdx))) {
