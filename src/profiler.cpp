...
void Profiler::appendRecord(std::string kernelName, Phase phase,
...
Profiler::~Profiler() {
  ...
  std::for_each(this->records.begin(), this->records.end(),
                [](const ProfilerRecord &r) {
  ...
  auto totalLowering =
      timeLoweringShapeInfer + timeLoweringToStableHLO + timeLoweringExtra;
  ...
  auto timeExecutionBufferPrepare =
      perPhaseTimes[Phase::EXECUTION_BUFFER_PREPARE];
  auto timeExecutionRun = perPhaseTimes[Phase::EXECUTION_RUN];
  auto timeExecutionBufferClearUp =
      perPhaseTimes[Phase::EXECUTION_BUFFER_CLEARUP];
  auto totalExecution = timeExecutionBufferPrepare + timeExecutionRun +
                        timeExecutionBufferClearUp;
  ...
  auto timeLeft = timeTotal - totalLowering - perPhaseTimes[Phase::JITCOMPILE] -
                  totalExecution;
...
///
///
ProfilerRecorder::ProfilerRecorder(std::string kernelName, Phase phase) {
...
ProfilerRecorder::~ProfilerRecorder() {
  ...
  auto duration =
      std::chrono::duration<double, std::micro>(now - this->start).count();
