...
static void
preprocWithExistingPasses(OpBuilder opBuilder, PassManager &pm,
                          func::FuncOp funcOp,
...
void inferShape(MLIRContext *ctx, ModuleOp moduleOp, int64_t NumHostArgs,
  ...
  // Key: index of the arguments of the function, value: if the argment is
  // literal type, we know the value of the arg
  ...
  moduleOp.walk([&](func::FuncOp funcOp) {
    assert(NumHostArgs == funcOp.getNumArguments() &&
           "NumHostArgs is not equal to funcOp args count!!");
  ...
  for (auto funcOp : moduleOp.getOps<func::FuncOp>()) {
    for (int i = 0; i < funcOp.getNumArguments(); i++) {
      if (shapeConstMap.contains(i)) {
        valueMap.insert(
            std::pair<Value, int>(funcOp.getArgument(i), shapeConstMap.at(i)));
      }
    }
